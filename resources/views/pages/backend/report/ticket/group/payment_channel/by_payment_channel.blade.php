...
@section('title', __('label.report_by_payment_channel'))
@section('content')
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover" id="paymentChannelTable" width="100%" cellspacing="0">
                    <thead class="thead-light">
                        <tr>
                            <th width="20%">{{ __('label.channel') }}</th>
...
@push('styles')
    <style>
        #paymentChannelTable tbody tr:hover {
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('#paymentChannelTable tbody').on('click', 'tr', function () {
                const data = table.row(this).data();
                if (data && data.uid) {
                    window.location.href = "{{ route('backend.report.list_by_payment_channel', '__uid__') }}".replace('__uid__', data.uid);
                }
            });
        });
    </script>
