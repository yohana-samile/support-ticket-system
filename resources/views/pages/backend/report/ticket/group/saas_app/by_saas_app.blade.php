...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('#saasAppTable tbody').on('click', 'tr', function () {
                const data = table.row(this).data();
                if (data && data.uid) {
                    window.location.href = "{{ route('backend.report.list_by_saas_app', '__uid__') }}".replace('__uid__', data.uid);
                }
            });
        });
    </script>
