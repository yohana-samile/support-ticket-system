    ...
    <script>
        $(document).ready(function () {
            ...
            function performExport() {
                let uid = exportScope === 'current' ? $('#saasSelector').val() : 'all';
                let start_date = $('#startDate').val();
                let end_date = $('#endDate').val();

                if (!uid && exportScope === 'current') {
                    toastr.info('Please select a saas first', '', {
                        timeOut: 3000,
                ...
                // Prepare export URL
                const url = "{{ route('backend.report.export_ticket_by_saas_app') }}" +
                    '?saas=' + encodeURIComponent(uid) +
                ...
                setTimeout(() => {
                    ...
                    toastr.success(`Exporting ${exportScope === 'all' ? 'all saas data' : 'current view'} as ${exportType.toUpperCase()}`, 'Export Started', {
                        timeOut: 3000,
                        positionClass: 'toast-bottom-right'
                    });
                }, 2000);
