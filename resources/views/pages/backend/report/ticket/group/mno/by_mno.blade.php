...
@section('title', __('label.report_by_mno'))
@section('content')
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-hover" id="mnoTable" width="100%" cellspacing="0">
                    <thead class="thead-light">
                        <tr>
                            <th width="20%">{{ __('label.mno') }}</th>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            table = $('#mnoTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('backend.report.mno_data') }}",
                    data: function (d) {
                ...
                language: {
                    search: "_INPUT_",
                    searchPlaceholder: "Search MNOs...",
                },
            });

            // Click event for rows
            $('#mnoTable tbody').on('click', 'tr', function () {
                const data = table.row(this).data();
                if (data && data.uid) {
                    window.location.href = "{{ route('backend.report.list_by_mno', '__uid__') }}".replace('__uid__', data.uid);
                }
            });
        });
    </script>
