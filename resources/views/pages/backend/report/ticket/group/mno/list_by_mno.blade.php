...
@section('title', __('label.tickets_by_mno'))
@section('content')
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <select id="mnoSelector" class="form-control mb-3" style="width: 300px;">
                @foreach($operators as $operator)
                    <option value="{{ $operator->uid }}" {{ request()->route('mno') == $operator->uid ? 'selected' : '' }}>
                        {{ $operator->name }}
                    </option>
                @endforeach
            </select>
            <div class="table-responsive">
                <table class="table table-bordered table-hover" id="listTicketsByMnoTable" width="100%" cellspacing="0">
    ...
    <script>
        $(document).ready(function () {
            function loadTickets(uid) {
                if ($.fn.DataTable.isDataTable('#listTicketsByMnoTable')) {
                    table.destroy();
                }

                table = $('#listTicketsByMnoTable').DataTable({
                    processing: true,
                    serverSide: false,
                    ajax: {
                        url: "{{ route('backend.report.list_by_mno', '__uid__') }}".replace('__uid__', uid),
                        data: function (d) {
            ...
            // Initial load
            let initialUid = $('#mnoSelector').val();
            loadTickets(initialUid);

            // Reload when selector changes
            $('#mnoSelector').on('change', function () {
                let uid = $(this).val();
                loadTickets(uid);
            });
            ...
            function performExport() {
                let uid = exportScope === 'current' ? $('#mnoSelector').val() : 'all';
                let start_date = $('#startDate').val();
                let end_date = $('#endDate').val();

                if (!uid && exportScope === 'current') {
                    toastr.info('Please select a operator first', '', {
                        timeOut: 3000,
                ...
                // Prepare export URL
                const url = "{{ route('backend.report.export_ticket_by_mno') }}" +
                    '?operator=' + encodeURIComponent(uid) +
                ...
                setTimeout(() => {
                    ...
                    toastr.success(`Exporting ${exportScope === 'all' ? 'all operator data' : 'current view'} as ${exportType.toUpperCase()}`, 'Export Started', {
                        timeOut: 3000,
                        positionClass: 'toast-bottom-right'
                    });
                }, 2000);
