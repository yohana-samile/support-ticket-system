    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-hover" id="topicTable" width="100%" cellspacing="0">
                    <thead class="thead-light">
                        <tr>
                            <th width="20%">{{ __('label.topic') }}</th>
    ...
    <script>
        $(document).ready(function() {
            table = $('#topicTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('backend.report.topic_data') }}",
                    data: function (d) {
            ...
            $('#topicTable tbody').on('click', 'tr', function () {
                ...
                if (data && data.uid) {
                    window.location.href = "{{ route('backend.report.list_by_topic', '__uid__') }}".replace('__uid__', data.uid);
                }
            });
        });
    </script>
