...
@section('title', __('label.tertiary_topic_information'))
@section('content')
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-8">
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">{{__('label.tertiary_topic_information')}}</h6>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('backend.tertiary.update', $tertiaryTopic->uid) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="topic_id" class="form-label">{{__('label.main_topic')}} <span class="text-danger">*</span></label>
                                <select class="form-control select2-ajax" id="topic_id" name="topic_id"
                                        data-placeholder="{{ __('Search main topic...') }}"
                                        data-ajax-url="{{ route('backend.topic.search') }}"
                                        required>
                                    @if($tertiaryTopic->subtopic && $tertiaryTopic->subtopic->topic)
                                        <option value="{{ $tertiaryTopic->subtopic->topic->id }}" selected>
                                            {{ $tertiaryTopic->subtopic->topic->name }}
                                        </option>
                                    @endif
                                </select>
                                @error('topic_id')
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2-ajax" id="sub_topic_id" name="sub_topic_id"
                                        ...
                                        data-depends-on="#topic_id"
                                        required>
                                    @if($tertiaryTopic->subtopic)
                                        <option value="{{ $tertiaryTopic->subtopic->id }}" selected>
                                            {{ $tertiaryTopic->subtopic->name }}
                                        </option>
                                    @endif
                                </select>
                                @error('sub_topic_id')
                            ...
                            <div class="form-group">
                                <label for="name" class="form-label">{{__('label.tertiary_topic_name')}} <span class="text-danger">*</span></label>
                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $tertiaryTopic->name) }}" required>
                                @error('name')
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" id="description" name="description" rows="3">{{ old('description', $tertiaryTopic->description) }}</textarea>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" id="is_active"
                                           name="is_active" value="1" {{ $tertiaryTopic->is_active ? 'checked' : '' }}>
            ...
            <div class="col-lg-4">
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">{{__('label.tertiary_topic_details')}}</h6>
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                            <strong>{{__('label.main_topic')}}:</strong>
