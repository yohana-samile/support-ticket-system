            ...
            <div class="col-lg-8">
                <div class="card shadow mb-4">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary">{{__('label.topic_information')}}</h6>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('backend.topic.update', $topic->uid) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                ...
                                <select class="form-control select2-ajax" id="saas_app_id" name="saas_app_id"
                                    ...
                                    <option value=""></option>
                                    @if($topic->saas_app_id || old('saas_app_id'))
                                        <option value="{{ old('saas_app_id', $topic->saas_app_id) }}" selected>
                                            {{ old('saas_app_name', $topic->saasApp->name ?? '') }}
                                        </option>
                                    @endif
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <div class="custom-control custom-switch">
                                    <input type="checkbox" class="custom-control-input" id="is_active"
                                           name="is_active" value="1" {{ $topic->is_active ? 'checked' : '' }}>
