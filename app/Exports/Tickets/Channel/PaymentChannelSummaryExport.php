...
class PaymentChannelSummaryExport implements FromQuery, WithHeadings, WithMapping, WithTitle
{
    protected $filters;
    ...
    public function map($channel): array
    {
        ...
        foreach (Status::orderBy('name')->get() as $status) {
            $statusCounts[] = $channel->{"{$status->slug}_tickets_count"};
        }
    ...
    {
        return 'Payment Channels Summary';
    }
}
